...
class AllienInavasion:
    ...
    def __init__(self):
        ...
        self.ship = Ship(self, 'images/ship.bmp')
        self.bullets = pygame.sprite.Group()
        self.aliens = pygame.sprite.Group()
        a = ('images/space.jpg', 'images/space1.jpg')
        self.background = Background(random.choice(a), [0, 0])
        self._create_fleet()
        self.logo = Logo([500, 0])
        self.play_button = Button(self, "PLAY", 770, 450, 200, 50)
        self.play_button_easy = Button(self, "PLAY ON EASY DIFFICULTY", 770, 400, 480, 50)
        self.play_button_hard = Button(self, "PLAY ON HARD DIFFICULTY", 770, 500, 480, 50)
        #self.lenguage = "English"
        #self.lenguage_button = Button(self, self.lenguage, 1425, 825, 200, 50)
        self.play_hint = Hint(self, "Натисніть P щоб перейти до вибору складності")
        self.difficulty_button_active = False
    ...
    def _check_play_button(self, mouse_pos):
            if self.play_button.rect.collidepoint(mouse_pos):
                self.difficulty_button_active = True
            elif self.play_button_easy.rect.collidepoint(mouse_pos):
    ...
    def _update_screen(self):
        ...
        if not self.stats.game_active:
            ...
            #self.lenguage_button.draw_button()
            if not self.difficulty_button_active:
                self.play_button.draw_button()
            if self.difficulty_button_active:
                self.play_button_easy.draw_button()
                self.play_button_hard.draw_button()
            if not self.difficulty_button_active:
                self.play_hint.draw_hint()
        pygame.display.flip()
    ...
    def _check_bullet_alien_collisions(self):
        ...
        if colisions:
            a = pygame.mixer.Sound("sound/explosion.mp3")
            a.play()
            for aliens in colisions.values():
                self.stats.score = int(self.stats.score) + self.settings.alien_point * len(aliens)
            self.sb.prep_score()
    ...
    def _update_aliens(self):
        ...
        self.aliens.update()
        # шукати зіткнення з прибульців з кораблем
    ...
    def _check_aliens_bottom(self):
        """Перевірити чи прибулець досяг низу екрану"""
